...
| `session/request_permission` | Offers one-shot allow/reject choices for bridge-owned approval requests carrying a tool call id. Clients may answer automatically; every answer is recorded as `external-client/acp`, never as an interactive user. |
