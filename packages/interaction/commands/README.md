...
`execute()` is the existing Remote/legacy entrypoint and records `source: { kind: 'unattributed' }`; old `{ kind: 'user' }` records remain readable but prove no actor. A trusted static Host adapter calls `createCommandIngress(owner, actor)` and passes the returned opaque, owner-scope-limited capability to `executeTrustedCommand(ingress, agent, line, images, signal)`, which records `source: { kind: 'interaction', actor }`. Neither helper is a `ctx.commands` method or dynamic Cordis catalog capability, and actor data is never a command wire parameter. `ctx.commands.guard()` installs global or `agent.ctx`-scoped monotonic synchronous denials; after image admission and every other asynchronous work, the runtime applies those guards and checks the receiving live session's captured required-extension scope immediately before invoking the handler. An unavailable extension may still produce the `command/run`/error `command/done` audit pair, but the handler does not run.
