...
`execute()` 是既有的 Remote／legacy 入口，记录 `source: { kind: 'unattributed' }`；旧的 `{ kind: 'user' }` 记录仍可读取，但不能证明 actor。受信任的静态宿主适配器调用 `createCommandIngress(owner, actor)`，并将返回的不透明、受 owner 作用域限制的 capability 传给 `executeTrustedCommand(ingress, agent, line, images, signal)`；后者记录 `source: { kind: 'interaction', actor }`。这两个 helper 都不是 `ctx.commands` 方法或动态 Cordis catalog capability，actor 数据也绝不是命令 wire 参数。`ctx.commands.guard()` 安装全局或 `agent.ctx` 作用域的单调同步拒绝；图片准入和其他所有异步工作完成后，运行时会应用这些 guard，并在调用 handler 前立即检查接收方实时会话捕获的 required-extension 作用域。extension 不可用时仍可产生 `command/run`／错误 `command/done` 审计对，但 handler 不会运行。
